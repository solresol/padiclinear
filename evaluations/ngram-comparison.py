...
for tokenisation_method_id in ['unigram', 'bigram', 'tri_token']:
    data = pandas.read_sql(f"""
 select * 
 from machine_learning_morphology_scoring
 where bible_version_id = {args.bible_version_id}
and tokenisation_method_id = '{tokenisation_method_id}'
and calculation_algorithm = 'LocalPadicLinear'
and result_version = '{args.result_version}'
""", engine)
    data.set_index('algorithm_region_size_parameter'
                         ).sort_index().proportion_correct.plot(ax=ax,
                                                               label=f"{tokenisation_method_id}")
#ax.legend()
